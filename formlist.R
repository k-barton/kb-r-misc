...
function(x, env = parent.frame(),
    ...
    style <- if(!missing(style) && !anyNA(style)) match.arg(style) else
        if(inherits(x, "formlist") && !is.null(attr(x, "mform.style")))
            attr(x, "mform.style") else
                stop("'style' missing")
		
    
    #rhslist <- lapply(x, \(x) "[[", 2L)
    rhslist <- lapply(x, \(f) f[[length(f)]])
    
    fexpr <- switch(style, "|" =, bar = {
        #"bar separated"
        #if(is.null(response)) call("~", ff) else call("~", response, ff)
        ff <- Reduce(\(a,b) call("|", a, b), rhslist)
		addresponse(f = ff, y = response, env = env)
    }, "~" = , tilde =, {
        #"tilde separated"
		#rhslist[[1L]] <- if(is.null(response)) call("~", rhslist[[1L]]) else call("~", response, rhslist[[1L]])
        ...
        #f <- if(is.null(response)) call("~", ff) else call("~", response, ff)
...
function(x, env = environment(f)) {
    ...
    rval <- rev(rval)
    if(is.null(response)) {
        #rval <- lapply(rval, \(g) formula(call("~", g), env = env))
